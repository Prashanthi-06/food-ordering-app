...
def add_to_cart(food_id):
    ...
    flash(
        f"Added {food.name} to your cart.",
        "success"
    )
...
def checkout():
    ...
    if request.method == "POST":
        ...
        #
        # IMPORTANT:
        # MySQL order_item table contains menu_item_id,
        # NOT food_item_id or food_name.
        #
        for entry in items:

            order_item = OrderItem(
                order_id=order.id,
                menu_item_id=entry["food"].id,
                quantity=entry["qty"],
...
def seed():
    ...
    sample_foods = [
        ...
        (
            "Veg Biryani",
            "Basmati rice with mixed vegetables & spices",
            179,
            "Biryani"
        ),
        (
            "Cheese Burger",
            "Grilled patty with cheese, lettuce & tomato",
            149,
            "Burgers"
        ),
        (
            "Veggie Burger",
            "Crispy veggie patty with fresh toppings",
            129,
            "Burgers"
        ),
        (
            "Butter Chicken",
            "Creamy tomato curry with tender chicken",
            259,
            "Curries"
        ),
        (
            "Paneer Tikka Masala",
            "Grilled paneer in spiced tomato gravy",
            219,
            "Curries"
        ),
        (
            "Chocolate Brownie",
            "Warm fudgy brownie with chocolate sauce",
            99,
            "Desserts"
        ),
        (
            "Gulab Jamun (2 pcs)",
            "Soft milk dumplings in sugar syrup",
            69,
            "Desserts"
        ),
        (
    ...
    for name, desc, price, category in sample_foods:
        ...
        if not FoodItem.query.filter_by(
            name=name
        ).first():
